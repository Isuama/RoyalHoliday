...
{
    public partial class frmHotelSeason : Form
    {
        private const string msghd = "Hotel Season Details";
        public int Mode = 0; //TO GET TO KNOW WEATHER INSERTION OR UPDATION
        public int SystemCode = 0; // TO KEEP THE SYSTEM GENERATED CODE
        public string SqlQry = "SELECT ID,Code,Name AS [Season],IsActive From mst_HotelSeasons Where Isnull([Status],0)<>7 Order By Code";
        ...
        private void Generate_Season_Code()
        {
            SqlQry = "SELECT MAX(ID) AS ID FROM mst_HotelSeasons";
            DataTable DT = Tourist_Management.Classes.clsGlobal.objCon.Fill_Table(SqlQry);
            txtCode.Text = "SES" + (Convert.ToInt32(DT.Rows[0]["ID"]) + 1).ToString();
        }
        private void Fill_Details()
        {
            ...
            try
            {
                DT = Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("Select Code,Name,[From],[To],Description,Isnull(IsActive,0)as IsActive From mst_HotelSeasons Where ID=" + SystemCode + "");
                txtCode.Text = DT.Rows[0]["Code"].ToString();
                txtName.Text = DT.Rows[0]["Name"].ToString();
                txtDesc.Text = DT.Rows[0]["Description"].ToString();
                dtpFrom.Value=Convert.ToDateTime(DT.Rows[0]["From"].ToString());
                dtpTo.Value=Convert.ToDateTime(DT.Rows[0]["To"].ToString());
        ...
        private Boolean Validate_Data()
        {
                ...
                if (Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("Select Code From mst_HotelSeasons Where Code='" + txtCode.Text.Trim() + "' and ID <> " + SystemCode + "").Rows.Count > 0)
                {
                    MessageBox.Show("Code already exists", msghd, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }
                if (Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("Select Name From mst_HotelSeasons Where Name='" + txtName.Text.Trim() + "' and ID <> " + SystemCode + "").Rows.Count > 0)
                {
                    MessageBox.Show("Season Name already exists", msghd, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }
                return true;
        }
        ...
        private Boolean Save_Data()
        {
            ...
            Boolean RtnVal = false;
                ...
                sqlCom.CommandText = "spSave_HotelSeasons";
                ...
                sqlCom.Parameters.Add("@From", SqlDbType.DateTime).Value = dtpFrom.Value;
                sqlCom.Parameters.Add("@To", SqlDbType.DateTime).Value = dtpTo.Value;
        ...
        public frmHotelSeason(){InitializeComponent();}
        private void frmHotelSeason_Load(object sender, EventArgs e)
        {
            Intializer();
        }
