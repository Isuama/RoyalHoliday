...
namespace Tourist_Management.Master
{
    ...
    {
        private const string msghd = "Room Type Details";
        public int Mode = 0; //TO GET TO KNOW WEATHER INSERTION OR UPDATION
        public int SystemCode = 0; // TO KEEP THE SYSTEM GENERATED CODE
        public string SqlQry = "SELECT ID,Code,Name,IsActive From mst_RoomTypes Where Isnull([Status],0)<>7 Order By Code";
        ...
        private void Generate_RoomType_Code()
        {
            SqlQry = "SELECT MAX(ID) AS ID FROM mst_RoomTypes";
            DataTable DT = Tourist_Management.Classes.clsGlobal.objComCon.Fill_Table(SqlQry);
            txtCode.Text = "RMT" + (Convert.ToInt32(DT.Rows[0]["ID"]) + 1).ToString();
        }
        private void Fill_Details()
        {
            ...
            try
            {
                DT = Tourist_Management.Classes.clsGlobal.objComCon.Fill_Table("Select Code,Name,Description,Isnull(IsActive,0)as IsActive From mst_RoomTypes Where ID=" + SystemCode + "");
        ...
        private Boolean Validate_Data()
        {
                ...
                if (Tourist_Management.Classes.clsGlobal.objComCon.Fill_Table("Select Code From mst_RoomTypes Where Code='" + txtCode.Text.Trim() + "' and ID <> " + SystemCode + "").Rows.Count > 0)
                {
                    MessageBox.Show("Code already exists", msghd, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }
                if (Tourist_Management.Classes.clsGlobal.objComCon.Fill_Table("Select Name From mst_RoomTypes Where Name='" + txtName.Text.Trim() + "' and ID <> " + SystemCode + "").Rows.Count > 0)
                {
        ...
        private Boolean Save_Data()
        {
            ...
            Boolean RtnVal = false; 
                ...
                sqlCom.CommandText = "spSave_Room_Types";
        ...
        public frmRoomTypes(){InitializeComponent();}
