...
namespace Tourist_Management.Master
{
    ...
    {
        private const string msghd = "Vehicle Types";
        public int Mode = 0; //TO GET TO KNOW WEATHER INSERTION OR UPDATION
        public int SystemCode = 0; // TO KEEP THE SYSTEM GENERATED CODE
        public string SqlQry = "SELECT ID,Code,Category,Name,IsActive From mst_VehicleTypes Where Isnull([Status],0)<>7 Order By Code";
        ...
        {
            DataTable DT;
            DT = Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("SELECT DISTINCT Category as Category FROM mst_VehicleTypes");
            cmbCat.Items.Clear();
            foreach (DataRow dr in DT.Rows)
            {
                cmbCat.Items.Add(dr["Category"].ToString());
            }
            cmbCat.AutoCompleteMode = ((System.Windows.Forms.AutoCompleteMode)((System.Windows.Forms.AutoCompleteMode.Suggest | System.Windows.Forms.AutoCompleteMode.Append)));
        ...
        private void Fill_Details()
        {
            ...
            try
            {
                DT = Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("Select Code,Category,Name,Description,Isnull(IsActive,0)as IsActive From mst_VehicleTypes Where ID=" + SystemCode + "");
                txtCode.Text = DT.Rows[0]["Code"].ToString();
                cmbCat.Text = DT.Rows[0]["Category"].ToString();
        ...
        private Boolean Validate_Data()
        {
                ...
                if (cmbCat.Text.Trim() == "")
                {
                    MessageBox.Show("Category cannot be blank", msghd, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }
                ...
                if (Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("Select Code From mst_VehicleTypes Where Code='" + txtCode.Text.Trim() + "' and ID <> " + SystemCode + "").Rows.Count > 0)
                {
                    MessageBox.Show("Code already exists", msghd, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }
                if (Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("Select Name From mst_VehicleTypes Where Name='" + txtName.Text.Trim() + "' and ID <> " + SystemCode + "").Rows.Count > 0)
                {
        ...
        private Boolean Save_Data()
        {
            ...
            Boolean RtnVal = false;
                ...
                sqlCom.CommandText = "spSave_Vehicle_Types";
                ...
                sqlCom.Parameters.Add("@Category", SqlDbType.VarChar, 50).Value = cmbCat.Text.Trim();
        ...
        public frmVehicleType(){InitializeComponent();}
