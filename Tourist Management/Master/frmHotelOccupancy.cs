...
{
    public partial class frmHotelOccupancy : Form
    {
        private const string msghd = "Occupancy Details";
        public int Mode = 0; //TO GET TO KNOW WEATHER INSERTION OR UPDATION
        public int SystemCode = 0; // TO KEEP THE SYSTEM GENERATED CODE
        public string SqlQry = "SELECT ID,Code,Name AS [Occupancy],IsActive From mst_HotelOccupnacy Where Isnull([Status],0)<>7 Order By Code";
        ...
        private void Fill_Details()
        {
            ...
            try
            {
                DT = Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("Select Code,Name,Description,Isnull(IsActive,0)as IsActive From mst_HotelOccupnacy Where ID=" + SystemCode + "");
        ...
        private Boolean Validate_Data()
        {
                ...
                if (Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("Select Code From mst_HotelOccupnacy Where Code='" + txtCode.Text.Trim() + "' and ID <> " + SystemCode + "").Rows.Count > 0)
                {
                    MessageBox.Show("Code already exists", msghd, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }
                if (Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("Select Name From mst_HotelOccupnacy Where Name='" + txtName.Text.Trim() + "' and ID <> " + SystemCode + "").Rows.Count > 0)
                {
        ...
        private Boolean Save_Data()
        {
            ...
            Boolean RtnVal = false;
                ...
                sqlCom.CommandText = "spSave_HotelOccupancy";
        ...
        public frmHotelOccupancy(){InitializeComponent();}
        private void frmHotelOccupancy_Load(object sender, EventArgs e)
        {
            Intializer();
        }
