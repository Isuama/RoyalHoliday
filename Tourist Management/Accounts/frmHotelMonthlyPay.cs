...
{
    public partial class frmHotelMonthlyPay : Form
    {
        private const string msghd = "Hotel Monthly Due Payments";
        string SqlQuery;
        ...
        enum RP { gTID, gHID, gVID, gCNM, gHNM, gBNO, gRTI, gRBI, gROI, gRTY ,gBSS, gOCC, gNOR, gGRM, gFRM, gEBD, gEBC, gRTE, gGRT, gANO };
        enum MP { gTID, gHID, gVID, gCNM, gHNM, gBNO, gMNM, gAMC, gCMC, gGMC, gANO };
        public frmHotelMonthlyPay(){InitializeComponent();}
        private void frmHotelMonthlyPay_Load(object sender, EventArgs e)
        {
            Intializer();
        }
        ...
        private void Fill_Control()
        {
            try
            { 
                drpHotel.DataSource = Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("SELECT ID,Name AS HotelName FROM mst_HotelDetails Where ISNULL(PayID,1)=2 AND IsNull(IsActive,0)=1 ORDER BY ID");
          ...
          DataTable       DTB = Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("SELECT ID,Company_Logo FROM dbo.mst_CompanyGenaral WHERE ID=" + cmbCompany.SelectedValue + "");
                if (DTB.Rows[0]["Company_Logo"] != DBNull.Value)
                {
                    byte[] CompLogo = (byte[])DTB.Rows[0]["Company_Logo"];
        ...
        private void Grd_Initializer()
        {
            try
            {
                ...
                grdRoomPay.Cols.Count = 20;
                grdMealPay.Cols.Count = 11;
                ...
                grdDuePayDaily.Cols[(int)SP.gHPS].Width = 70;
                grdDuePayDaily.Cols[(int)SP.gCNM].Width = 201;
                grdDuePayDaily.Cols[(int)SP.gHNM].Width = 0;
                grdDuePayDaily.Cols[(int)SP.gDTE].Width = 66;
                grdDuePayDaily.Cols[(int)SP.gADL].Width = 41;
                grdDuePayDaily.Cols[(int)SP.gCHD].Width = 39;
                grdDuePayDaily.Cols[(int)SP.gGUD].Width = 41;
                grdDuePayDaily.Cols[(int)SP.gFCA].Width = 50;
                grdDuePayDaily.Cols[(int)SP.gFCC].Width = 50;
                grdDuePayDaily.Cols[(int)SP.gCID].Width = 0;
                grdDuePayDaily.Cols[(int)SP.gCUR].Width = 45;
                grdDuePayDaily.Cols[(int)SP.gGCI].Width = 0;
                grdDuePayDaily.Cols[(int)SP.gGCR].Width = 0;
                grdDuePayDaily.Cols[(int)SP.gCRT].Width = 80;
                grdDuePayDaily.Cols[(int)SP.gGCN].Width = 0;
                grdDuePayDaily.Cols[(int)SP.gADV].Width = 87;
                grdDuePayDaily.Cols[(int)SP.gCOM].Width = 87;
                grdDuePayDaily.Cols[(int)SP.gTOT].Width = 86;
                grdDuePayDaily.Cols[(int)SP.gPID].Width = 50;
                grdDuePayDaily.Cols[(int)SP.gCNF].Width = 52;
                grdDuePayDaily.Cols[(int)SP.gPPD].Width = 0;
                grdDuePayDaily.Cols[(int)SP.gNON].Width = 55;
                grdDuePayDaily.Cols[(int)SP.gDUE].Width = 89;
                ...
                grdMealPay.Cols[(int)MP.gBNO].Width = 100;
                ...
                grdRoomPay.Cols[(int)RP.gBNO].Caption = "Bill No";
                ...
                grdMealPay.Cols[(int)MP.gBNO].Caption = "Bill No";
        ...
        private void Filter_Values()
        {
            try
            {
                DataTable DT;
                string ssql,filter;
                #region MONTHLY PAYMENT DETAILS
                ...
                {                    
                    ...
                    ssql = "SELECT TransID,DateIn,DateOut,VoucherID,UniqueID,AmendNo,Guest," +
                               ...
                               "HandledBy,HotelID,HotelName,RoomTypeID,RoomBasisID,OccupancyID,RoomTypeName,RoomBasisName,Occupancy," +
                                 ...
                                 "IsNull(RoomCount,0)AS RoomCount,GuideRooms,IsNull(FOCRooms,0)AS FOCRooms,IsNull(Nights,1)AS Nights,IsNull(MealFor,'')MealFor," +
                                 ...
                                 " FROM vw_acc_HotelAllPayments WHERE ConfirmPaid<>1 ";
                if (txtTourNo.Text.ToString().Trim()!="")
                {
                    filter = " AND TransID=" + txtTourNo.Text.ToString().Trim() + "";
                }
                else if (chkTodayPay.Checked)
                {
                    filter = " AND PaidDate='" + today.Trim() + "' AND HotelID=" + HotelID + "";
                }
                else
                {
                    filter = " AND Dateout>='" + DateFrom.Trim() + "' AND Dateout<='" + DateTo.Trim() + "'" +
                             " AND HotelID=" + HotelID + "";                    
                }
                if (chkCmpny.Checked)
                {
                ...
                ssql = (ssql + filter + " ORDER BY Guest").Trim();                
                ...
                C1.Win.C1FlexGrid.CellStyle PAD = grdDuePayDaily.Styles.Add("PAD");
                ...
                var Bookings = from val in DT.AsEnumerable()
                               ...
                               select new
                               {
                                   ...
                                   ChequeNo = reserve.Max(id => id["ChkNo"]),
                                   HandleBy = reserve.Max(id => id["HandledBy"]),
                ...
                foreach (var items in Bookings)
                {
                    ...
                    if(count==1)
                        txtChkNo.Text = items.ChequeNo.ToString().Trim();
                ...
                var Rates = from val in DT.AsEnumerable()
                            ...
                            select new
                            {
                                ...
                                HotelID = reserve.Max(id => id["HotelID"]),
                                VoucherID = reserve.Max(id => id["VoucherID"]),
                                BillNo = reserve.Max(id => id["BillNoRoom"]),
                                Guest = reserve.Max(id => id["Guest"]),
                                HotelName = reserve.Max(id => id["HotelName"]),
                ...
                foreach (var items in Rates)
                {
                    ...
                    grdRoomPay[count, (int)RP.gBNO] = items.BillNo.ToString().Trim();
                ...
                var Meal = from val in DT.AsEnumerable()
                           ...
                           select new
                           {
                               ...
                               HotelID = reserve.Max(id => id["HotelID"]),
                               VoucherID = reserve.Max(id => id["VoucherID"]),
                               BillNo = reserve.Max(id => id["BillNoHotel"]),
                               Guest = reserve.Max(id => id["Guest"]),
                               HotelName = reserve.Max(id => id["HotelName"]),
                ...
                foreach (var items in Meal)
                {
                    ...
                    grdMealPay[count, (int)MP.gBNO] = items.BillNo.ToString().Trim();
        ...
        private Boolean Save_Tabs(System.Data.SqlClient.SqlCommand sqlCom)
        {
                if (grdDuePayDaily.Rows.Count > 1)
                    if (Save_Basic_Details(sqlCom) == false)
                        return false;
                if (grdRoomPay.Rows.Count > 1)
                    if (Save_Room_Details(sqlCom) == false)
                        return false;
                if (grdMealPay.Rows.Count > 1)
                    if (Save_Meal_Details(sqlCom) == false)
                        return false;
                return true;
        }
        private Boolean Save_Basic_Details(System.Data.SqlClient.SqlCommand sqlCom)
        {
            ...
            Boolean RtnVal = true; 
                ...
                sqlCom.CommandText = "spSave_Hotel_Monthly_Payments";
                RowNumb = 1;
                if (grdDuePayDaily.Rows.Count == 1)
                    return true;
                while (grdDuePayDaily[RowNumb, grdDuePayDaily.Cols[(int)SP.gVID].Index] != null)
                {
                    ...
                    if (grdDuePayDaily[RowNumb, (int)SP.gTOT] != null && grdDuePayDaily[RowNumb, (int)SP.gTOT].ToString() != "")
                        sqlCom.Parameters.Add("@Total", SqlDbType.Decimal).Value = Convert.ToDecimal(grdDuePayDaily[RowNumb, (int)SP.gTOT].ToString());
                    ...
                    if (Convert.ToBoolean(grdDuePayDaily[RowNumb, (int)SP.gPID]))
                    {
                        sqlCom.Parameters.Add("@ChkNo", SqlDbType.NVarChar, 50).Value = txtChkNo.Text.ToString().Trim();
                    ...
                    if (grdDuePayDaily[RowNumb, (int)SP.gPRI]+"".Trim()!="")
                    {
                        sqlCom.Parameters.Add("@Prepaid", SqlDbType.Int).Value = 1;
                        sqlCom.Parameters.Add("@PrepaidID", SqlDbType.Decimal).Value = Convert.ToDecimal(grdDuePayDaily[RowNumb, (int)SP.gPRI]);
                    }
                    ...
                    RowNumb++;
                    if(grdDuePayDaily.Rows.Count==RowNumb)
                        return RtnVal;
                }
                return RtnVal;
        }
        private Boolean Save_Room_Details(System.Data.SqlClient.SqlCommand sqlCom)
        {
            ...
            Boolean RtnVal = true; 
                sqlCom.CommandType = CommandType.StoredProcedure;
                sqlCom.CommandText = "spSave_Hotel_Monthly_Payments_R";
                RowNumb = 1;
                if (grdRoomPay.Rows.Count == 1)
                    return true;
                while (grdRoomPay[RowNumb, grdRoomPay.Cols[(int)RP.gVID].Index] != null)
                {
                    ...
                    if (grdRoomPay[RowNumb, (int)RP.gBNO] != null && grdRoomPay[RowNumb, (int)RP.gBNO].ToString() != "")
                        sqlCom.Parameters.Add("@BillNo", SqlDbType.NVarChar, 50).Value = grdRoomPay[RowNumb, (int)RP.gBNO].ToString();
        ...
        private Boolean Save_Meal_Details(System.Data.SqlClient.SqlCommand sqlCom)
        {
            ...
            Boolean RtnVal = true; 
                sqlCom.CommandType = CommandType.StoredProcedure;
                sqlCom.CommandText = "spSave_Hotel_Monthly_Payments_M";
                RowNumb = 1;
                if (grdMealPay.Rows.Count == 1)
                    return true;
                while (grdMealPay[RowNumb, grdMealPay.Cols[(int)MP.gVID].Index] != null)
                {
                    ...
                    if (grdMealPay[RowNumb, (int)MP.gBNO] != null && grdMealPay[RowNumb, (int)MP.gBNO].ToString() != "")
                        sqlCom.Parameters.Add("@BillNo", SqlDbType.NVarChar, 50).Value = grdMealPay[RowNumb, (int)MP.gBNO].ToString();
        ...
        { 
                ...
                #endregion
                #region SHOW REPORT
                if (e.Col == grdDuePayDaily.Cols[(int)SP.gCNM].Index || e.Col == grdDuePayDaily.Cols[(int)SP.gHPS].Index)
                {
        ...
        { 
                Classes.clsConnection sConnection = new Tourist_Management.Classes.clsConnection();
                string reptype = "HotelMonthlyPay";
                decimal tourid = 0; 
                string format = "yyyy-MM-dd";
                DateTime datefrom = dtpFromDate.Value;
                string DateFrom = datefrom.ToString(format).Substring(0, 10);
                ...
                ReportDocument ga_1 = new ReportDocument();
                if(drpHotel.SelectedValue==null || drpHotel.SelectedValue.ToString().Trim()=="")
                    return;
                int HotelID=Convert.ToInt32(drpHotel.SelectedValue);
                sql = "SELECT TransID,TourID,DateIn,DateOut,VoucherID,AmendNo,Guest," +
                       "ISNULL(BillNoHotel,'')AS BillNoHotel,ISNULL(BillNoRoom,'')AS BillNoRoom,ISNULL(ChkNo,'')AS ChkNo," +
                       "HandledBy,HotelID,HotelName,RoomTypeID,RoomBasisID,OccupancyID,RoomTypeName,RoomBasisName,Occupancy," +
                       "IsNull(FOCAdult,0)AS FOCAdult,IsNull(FOCChild,0)AS FOCChild," +
                       "IsNull(CurID,2)AS CurID,CurCode," +
                       ...
                       "InitiatedCost,InitiatedEbedCost,total," +
                       "CompID,DisplayName,Telephone,Fax,E_Mail,E_mailTo,UserName,UserGroupID,GroupName,Web,Postal_Addres,Company_Logo,Aname1,Ano1,MDname,MDno,AADname,AADno,UniqueID UniqueBookingID" +
                       " FROM vw_acc_HotelMonthlyPayments" +
                       " WHERE HotelID=" + HotelID + " AND PaidDate='" + today.Trim() + "' AND UserID = " + Convert.ToInt32(Classes.clsGlobal.UserID.ToString()) + "";
                if (chkCmpny.Checked)
                {
                    sql += " AND CompID=" + cmbCompany.SelectedValue + "";
                }
                sql += " ORDER BY HandledBy,DateIn";
                DataSets.ds_acc_MonthlyDueHotel DTG = new DataSets.ds_acc_MonthlyDueHotel();
                    ga = new Tourist_Management.Reports.HotelMonthlyPayments();
                    ga_1 = new Tourist_Management.Reports.HotelMonthlyPayments();
                DataTable DT = Tourist_Management.Classes.clsGlobal.objCon.Fill_Table(sql);
                if (DT.Rows.Count > 0)
                {
                    sConnection.Print_Report(tourid.ToString(), sql, DTG, ga, reptype, new SqlParameter("comp", chkCmpny.Checked), new SqlParameter("sig",true  ));
                    DTG.Clear();
                    sConnection.Print_Report(tourid.ToString(), sql, DTG, ga_1, reptype, new SqlParameter("comp", chkCmpny.Checked), new SqlParameter("sig",false ));
                }
                else
        ...
        private void Print_Bank_Details()
        {
                ...
                string sql = "SELECT ID,HotelName,BankName,BranchName,Account,AccountNo" +
                             " FROM vw_HotelBankDetails WHERE ID=" + HotelString + "";
        ...
        {
                bool IsChecked = false;
                if (checkBox1.Checked)   IsChecked = true;
                int RowNumb = 1;
                while (grdDuePayDaily.Rows.Count > RowNumb)
                {
                    if (grdDuePayDaily[RowNumb, (int)SP.gVID] == null || grdDuePayDaily[RowNumb, (int)SP.gVID].ToString() == "")
        ...
        {
            drpHotel.setSelectedValue(null);
            string sql;
            ...
            {
                Filter_Values();
                grdDuePayDaily.Cols[(int)SP.gCNM].Width = 300;
                grdDuePayDaily.Cols[(int)SP.gHNM].Width = 290;
                grdMealPay.Cols[(int)MP.gCNM].Width = 0;
                grdMealPay.Cols[(int)MP.gHNM].Width = 140;
                grdRoomPay.Cols[(int)RP.gCNM].Width = 0;
                grdRoomPay.Cols[(int)RP.gHNM].Width = 142;
                if (grdDuePayDaily.Rows.Count > 1)
                {
                    lblClient.Text = grdDuePayDaily[1, (int)SP.gCNM]+"".Trim();
                }
                drpHotel.Enabled = false;
        ...
        {
            grdDuePayDaily.Cols[(int)SP.gCNM].Width = 290;
            grdDuePayDaily.Cols[(int)SP.gHNM].Width = 0;
            grdMealPay.Cols[(int)MP.gCNM].Width = 140;
            grdMealPay.Cols[(int)MP.gHNM].Width = 0;
            grdRoomPay.Cols[(int)RP.gCNM].Width = 142;
        ...
        private void Send_Email()
        {
            try
            {
                ...
                string path = "C:\\Temp\\HotelPayments\\HotelMonthlyPayments.pdf";
                string lFileName = path;
                oAttach = oMsg.Attachments.Add(@path, iAttachType, iPosition, sDisplayName);
                oMsg.Subject = "Hotel Monthly Payment";
        ...
        private void chkEbed_CheckedChanged(object sender, EventArgs e)
        {
            if (chkEbed.Checked)
            {
                grdRoomPay.Cols[(int)RP.gEBC].Width = 68;
                grdRoomPay.Cols[(int)RP.gEBD].Width = 40;
            }
            else
            {
                grdRoomPay.Cols[(int)RP.gFRM].Width = 0;
                grdRoomPay.Cols[(int)RP.gEBD].Width = 0;
            }
        }
        private void selected_Row_Colour_Change(string voucherID)
        {
            try
            {
                C1.Win.C1FlexGrid.CellStyle SEL = grdDuePayDaily.Styles.Add("SEL");
