...
{   
    public partial class frmSupplementaryCRN : Form
    {
        ...
        public frmSupplementaryCRN(){InitializeComponent();}
        ...
        private void Fill_Data()
        {
            try
            {
                ...
                SqlQry = "SELECT DISTINCT InvoiceNo,TransID,InvoiceNo,IsCash,[Date],ChkNo,ChkDate,CurrencyID,Currency," +
                       ...
                       " FROM vw_Supplementary_CRN WHERE TransID=" + SystemCode + " AND Type='CRN' ";
        ...
        private void Increase_Invoice_No()
        {
            ...
            if (txtTourNo.Text.ToString().Trim() != "")
            {
                int maxNO = Convert.ToInt32(Tourist_Management.Classes.clsGlobal.objCon.Fill_Table("SELECT COUNT(*) FROM dbo.act_SupplementaryInvoice WHERE Type='CRN' AND TransID=" + SystemCode + "").Rows[0][0].ToString());
                    txtInvoiceNo.Text = txtTourNo.Text.Trim() + "/C" + (maxNO + 1);
            }
        }
        ...
        private Boolean Validate_Data()
        {
                ...
                {
                    MessageBox.Show("'CRN NUMBER' Cannot Be Blank", msghd, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }
                if (cmbInvoiceTo.SelectedItem == null || cmbInvoiceTo.SelectedItem.ToString().Trim() == "")
                {
                    MessageBox.Show("'Credit Note To' Cannot Be Blank", msghd, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return false;
                }
        ...
        private Boolean Save_Data()
        {
            ...
            Boolean RtnVal = false;
                ...
                while (grdPayments[RowNumb, grdPayments.Cols[(int)IG.gTID].Index] != null)
                {
                    ...
                    sqlCom.Parameters.Add("@Type", SqlDbType.VarChar, 10).Value = "CRN";
        ...
        {
            db.showReport(new Tourist_Management.Reports.rpt_acc_SupplementaryCRN(), SystemCode, Classes.clsGlobal.UserID);
                 } 
    }
}
