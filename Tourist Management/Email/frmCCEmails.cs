...
{
    public partial class frmCCEmails : Form
    {
        private const string msghd = "CC Emails Setup";
        public string SqlQry = "SELECT Email,IsActive From mst_CCEmails WHERE UserID=" + Convert.ToInt32(Classes.clsGlobal.UserID.ToString()) + " Order By SrNo";
        public int Mode = 0; //TO GET TO KNOW WHETHER INSERTION OR UPDATION
        public int SystemCode = 0; // TO KEEP THE SYSTEM GENERATED CODE
        Boolean bLoad = false;
        enum CC { gEAD,gIAT };
        public frmCCEmails(){InitializeComponent();}
        private void frmCCEmails_Load(object sender, EventArgs e)
        {
            Intializer();
        }
        ...
        private void Grd_Initializer()
        {
            try
            {
                ...
                grdCC.Cols[(int)CC.gEAD].Caption = "Email Address";
                grdCC.Cols[(int)CC.gIAT].Caption = "ADD";
                grdCC.Rows[1].AllowEditing = true;
                grdCC.Cols[(int)CC.gEAD].Width = 470;
                grdCC.Cols[(int)CC.gIAT].Width = 62;
        ...
        private void Fill_Data()
        {
            try
            {
                ...
                string ssql; 
                int RowNumb;
                ssql = "SELECT UserName FROM mst_UserMaster Where ID=" + Convert.ToInt32(Classes.clsGlobal.UserID.ToString()) + "";
                txtUserName.Text = Tourist_Management.Classes.clsGlobal.objComCon.Fill_Table(ssql).Rows[0]["UserName"].ToString();
                ssql = " SELECT Email,IsActive FROM dbo.mst_CCEmails WHERE UserID=" + Convert.ToInt32(Classes.clsGlobal.UserID.ToString()) + "";
                ...
                else
                {
                    ...
                    if (DT.Rows[0][0].ToString() != null && DT.Rows[0][0].ToString() != "")
                    {
                        ...
                        {
                            if (DT.Rows[RowNumb]["Email"].ToString() != "")
                                grdCC[RowNumb + 1, (int)CC.gEAD] = DT.Rows[RowNumb]["Email"].ToString();
        ...
        private Boolean Save_CCEmails(System.Data.SqlClient.SqlCommand sqlCom)
        {
            ...
            Boolean RtnVal = true;
                ...
                sqlCom.CommandText = "spSave_CCEmails";
                RowNumb = 1;
                while (grdCC[RowNumb, grdCC.Cols[(int)CC.gEAD].Index] != null)
                {
                    ...
                    if (grdCC[RowNumb, (int)CC.gEAD] != null)
                        sqlCom.Parameters.Add("@Email", SqlDbType.NVarChar, 200).Value = grdCC[RowNumb, (int)CC.gEAD].ToString();
                    if (grdCC[RowNumb, (int)CC.gEAD] != null)
